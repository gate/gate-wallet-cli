...
export function registerOpenApiCommands(program: Command) {
  ...
  program
    ...
    .action(
      async (opts: { setAk?: string; setSk?: string; setQueryAk?: string; setQuerySk?: string }) => {
      ...
      async (opts: {
        ...
        try {
          const params: Record<string, unknown> = {
            ...
            token_out: opts.to,
            slippage: String(
              opts.slippage ?? config?.default_slippage ?? 0.03,
            ),
    ...
    .action(
      async (opts: {
        ...
        if (!opts.signedTx && !opts.txHash) {
          console.error(
            chalk.red("--signed-tx 和 --tx-hash 必须传入其中一个"),
          );
          return;
        }
  ...
  program
    ...
    .action(
      async (opts: { chain: string; address: string; lan: string }) => {
        const chainId = resolveChainId(opts.chain);
        if (!chainId) {
          console.error(chalk.red(`未知链: ${opts.chain}`));
          return;
  ...
  program
    ...
    .action(
      async (opts: { chain: string; address: string; pair?: string }) => {
        const chainId = resolveChainId(opts.chain);
        if (!chainId) {
          console.error(chalk.red(`未知链: ${opts.chain}`));
          return;
